...
void main() {
  ...
  group('LanguageCodes.tryResolveFromLocale', () {
    test('exact match (language + script + country)', () {
      final locale = Locale.fromSubtags(
        languageCode: 'zh',
        scriptCode: 'Hans',
      ...
      expect(
        LanguageCodes.tryResolveFromLocale(locale),
        equals(LanguageCodes.zh_Hans_CN),
      );
    });
    ...
    test('orElse used when no match found', () {
      final locale = const Locale('xxx', 'YYY');
      final result = LanguageCodes.tryResolveFromLocale(
        locale,
        orElse: () => LanguageCodes.und,
      );
      expect(result, equals(LanguageCodes.und));
    ...
    test('returns null when no match found and no orElse provided', () {
      final locale = const Locale('xxx', 'YYY');
      expect(LanguageCodes.tryResolveFromLocale(locale), isNull);
    });
  });
}
